...
```{r read for math courses and earned hours}
...
add.vari.enth.coutn <- merge(APR_data.apr, stu_enroll, by="Stu_UWFID", all.x = T) %>% 
    ...
    mutate(EthnicityCdoe = ifelse(Stu_EthnicityCode == 5, "African American",
...
addmargins(table(add.vari.enth.coutn$Stu_Ethnicity, add.vari.enth.coutn$EthnicityCdoe))
```
...
mydata <- read_csv("ftic.enc.crs.math.csv") %>% 
    group_by(Stu_UWFID) %>% mutate(Num_Mathcrs = row_number()) %>% 
        ...
        mutate(TermGPA = ifelse(TermGPA == 0, NA, TermGPA))
...
apr_varis <- c("APR", "Stu_CollegeCode","AVE_HS_APR","CIP_APR","GPA_HIGHSCHOOL","Stu_Gender","Prior_hrs",
               ...
               "Term1.EaredHours" ,"EthnicityCdoe","County_Group","StateCode",  "Cohort" )
library(gtsummary)
...
addmargins(table(ft2017$APR, ft2017$Prior_hrs))
...
#ACCURACY
ACC_DF <- NULL
ACC_DF$Cohort <- c(2017:2020)
ACC_DF$ACC_TRAIN <- c(acc_f(tab_2017_train),acc_f(tab_2018_train),acc_f(tab_2019_train),acc_f(tab_2020_train))
ACC_DF$ACC_TEST <- c(acc_f(tab_2017_test),acc_f(tab_2018_test),acc_f(tab_2019_test),acc_f(tab_2020_test))
...
```{r}
apr_varis.1stterm <- c("APR", "Stu_CollegeCode","AVE_HS_APR","CIP_APR", "Stu_Gender","Prior_hrs","GPA_HIGHSCHOOL","CUMTGPUWF","County_Group","EthnicityCdoe","Term1.EaredHours","Cohort")
...
#filter ftic2021
not.ft2021.1stterm <- apr_1st.term.data %>% filter(Cohort != 2021 ) %>% select(-Cohort)
ft2021.1stterm <- apr_1st.term.data %>% filter(Cohort == 2021 ) %>% select(-Cohort)
#data partitions
...
### Prediction for Dropouts

 
```{r math course coding}
AA <- c("A","A-")
BB <- c("B+","B","B-")
CC <- c("C-","C","C+")
Failed <- c("D","D+","F","NF")
...
apr_varis.1stterm.dropouts <- c("APR", "Stu_CollegeCode","AVE_HS_APR","CIP_APR", "Stu_Gender","Prior_hrs","GPA_HIGHSCHOOL","CUMTGPUWF","County_Group","EthnicityCdoe","Term1.EaredHours", "Math_Grade_Range","MathCRS.Level",  "Cohort")
apr_1st.term.data.math  <- mydata.state.county.math[,-1] %>% 
    mutate(Dropout = ifelse(is.na(CIP_2ndFall), "Dropped","Stayed")) %>% 
...
#modeling
library(naivebayes)
dropout.1stterm <- naive_bayes(Dropout~., data = ft.1st.train.math, usekernel = TRUE)
...
PROB_DROPOUT <- predict(dropout.1stterm, ft2021.1stterm.math, type="prob") %>% data.frame()
PROB_DROPOUT_1 <- round(PROB_DROPOUT, 4)
output_ftic2021.pre.1st.drop <- cbind(PROB_DROPOUT_1, output_ftic2021.pre.1st) %>% 
...
head(output_ftic2021.pre.1st.drop, 20)
write.csv(output_ftic2021.pre.1st.drop, "output_ftic2021.pre.1st.drop.csv",row.names = F)
...
summary(apr.glm.2017)
summary(apr.glm.2018)
summary(apr.glm.2019)
summary(apr.glm.2020)
```


```{r}
exp(coef(apr.glm.2017))
P_value <- broom::tidy(apr.glm.2017)
# confnt
confint(apr.glm.2020, level = 0.95)
#coefficient
coef1 <- exp(coef(apr.glm.2017))
coef_table <- knitr::kable(coef1)
coef_table
vari_imp <- knitr::kable(caret::varImp(apr.glm.2017))
vari_imp
